...
include "../backend/function_loggedin.php";
    ...
    if(isset($_POST['submit'])){
        ...
        if ($data1 != "" && $data2 != "" && $data3 != ""  ){
            ...
            $sql_insert_datarow_1 = "INSERT INTO school_community_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','8.1.1',$data1,'{$_SESSION['username']}')";

            $sql_insert_datarow_2 = "INSERT INTO school_community_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','8.1.2',$data2,'{$_SESSION['username']}')";

            $sql_insert_datarow_3 = "INSERT INTO school_community_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','8.1.3',$data3,'{$_SESSION['username']}')";
...
$marks1 = "";
$sql = "SELECT  Marks FROM school_community_main WHERE Activity_Number ='8.1.1' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
...
$marks2 = "";
$sql = "SELECT  Marks FROM school_community_main WHERE Activity_Number ='8.1.2' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
$result = $con->query($sql);
...
$marks3 = "";
$sql = "SELECT  Marks FROM school_community_main WHERE Activity_Number ='8.1.3' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
$result = $con->query($sql);
...
<p>06 marks - Excellent </p>
  ...
  <tr class="active-row" >
    <td>8.1.1</td>
    <td>  Class circles are established in all classes, meetings are held at least 
once a term and decisions taken at class circles are implemented and 
records are maintained</td>
    ...
    <tr class="active-row" >
    <td>8.1.2</td>
    <td> Parents are made aware of the curriculum, new trends, projects, 
evaluation and circulars in class circles</td>
  ...
  <tr class="active-row" >
    <td>8.1.3</td>
    <td>Parents are made aware of each student’s progress, obtaining 
their ideas and they are encouraged to be involved in feedback and feed 
forward about their child</td>
    ...
    <?php $_SESSION["school_8.1"] = $total_marks; ?>
    <?php echo "Marks Total"."  ". $_SESSION["school_8.1"]; ?>
  </tr>
...
<a  style="float:right; color: black"; href= "school.8.2.php" >Go to next page</a>

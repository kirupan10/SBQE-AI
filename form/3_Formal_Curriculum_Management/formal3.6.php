<?php 
    ...
    if(isset($_POST['submit'])){
        ...
        if ($data1 != "" && $data2 != "" ){
            $sql_insert_datarow_1 = "INSERT INTO formal_curriculum_management_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','3.6.1',$data1,'{$_SESSION['username']}')";

            $sql_insert_datarow_2 = "INSERT INTO formal_curriculum_management_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','3.6.2',$data2,'{$_SESSION['username']}')";
...
$sql = "SELECT  Marks FROM formal_curriculum_management_main WHERE Activity_Number ='3.6.1' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
...
$sql = "SELECT  Marks FROM formal_curriculum_management_main WHERE Activity_Number ='3.6.2' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
$result = $con->query($sql);
...
<p>06 marks - Excellent </p>
  ...
  <tr class="active-row" >
    <td>3.6.1</td>
    <td> Responsibility regarding textbooks, syllabi, and teacher 
instructional manuals is entrusted to a particular teacher and 
Identification of the requirement and take necessary steps to 
fulfill them.</td>
    ...
    <tr class="active-row" >
    <td>3.6.2</td>
    <td> Textbooks, syllabi, and teacher instructional manuals are 
recorded, stored properly, all are appropriately distributed and 
actions are taken for taking back.</td>
   <td><input type="number"  autocomplete="off" name="marks2" value="<?php echo $marks2; ?>" required ></td>
    ...
    <?php $_SESSION["formal3_6"] = $total_marks; ?>
    <?php echo "Marks Total"."  ". $_SESSION["formal3_6"]; ?>
  </tr>
...
<a  style="float:right; color: black"; href="formal3.7.php" >Go to next page</a>
<a  style="float:left; color: black;" href="formal3.5.php" >Go to previous page</a>  

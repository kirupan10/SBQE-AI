<?php include "../backend/function_loggedin.php";
    ...
    if(isset($_POST['submit'])){
        ...
        if ($data1 != "" && $data2 != "" && $data3 != "" && $data4 != ""  ){
            ...
            $sql_insert_datarow_1 = "INSERT INTO physical_resource_management_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','7.2.1',$data1,'{$_SESSION['username']}')";

            $sql_insert_datarow_2 = "INSERT INTO physical_resource_management_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','7.2.2',$data2,'{$_SESSION['username']}')";

            $sql_insert_datarow_3 = "INSERT INTO physical_resource_management_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','7.2.3',$data3,'{$_SESSION['username']}')";

            $sql_insert_datarow_4 = "INSERT INTO physical_resource_management_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','7.2.4',$data4,'{$_SESSION['username']}')";
...
$marks1 = "";
$sql = "SELECT  Marks FROM physical_resource_management_main WHERE Activity_Number ='7.2.1' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
...
$marks2 = "";
$sql = "SELECT  Marks FROM physical_resource_management_main WHERE Activity_Number ='7.2.2' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
$result = $con->query($sql);
...
$marks3 = "";
$sql = "SELECT  Marks FROM physical_resource_management_main WHERE Activity_Number ='7.2.3' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
$result = $con->query($sql);
...
$marks4 = "";
$sql = "SELECT  Marks FROM physical_resource_management_main WHERE Activity_Number ='7.2.4' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
$result = $con->query($sql);
...
<p>06 marks - Excellent </p>
  ...
  <tr class="active-row" >
    <td>7.2.1</td>
    <td>  Different sections of the school are reserved for different 
purposes and the maximum use is made of all available resources and 
well maintained</td>
    ...
    <tr class="active-row" >
    <td>7.2.2</td>
    <td>Classrooms are maintained in an attractive and clean manner and a 
stimulating learning environment is created</td>
  ...
  <tr class="active-row" >
    <td>7.2.3</td>
    <td> Electricity and water are used very carefully and use is monitored</td>
    <td><input type="number"  autocomplete="off" name="marks3" value="<?php echo $marks3; ?>" required ></td>
  </tr>

    <tr class="active-row" >
    <td>7.2.4</td>
    <td>  Maintenance of physical environment in office</td>
   ...
   <td><input type="number"  autocomplete="off" name="" value="<?php echo $total_marks = $marks1 + $marks2 + $marks3 + $marks4; ?>" disabled ></td>
    <?php $_SESSION["physical.7_2"] = $total_marks; ?>
    <?php echo "Marks Total"."  ". $_SESSION["physical.7_2"]; ?>
  </tr>
...
<a  style="float:right; color: black"; href=" physical.7.3.php" >Go to next page</a>
<a  style="float:left; color: black;" href="physical.7.1.php" >Go to previous page</a>

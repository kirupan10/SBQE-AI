<?php include "../backend/function_loggedin.php";
    ...
    if(isset($_POST['submit'])){
        ...
        if ($data1 != "" && $data2 != "" ){
            $sql_insert_datarow_1 = "INSERT INTO physical_resource_management_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','7.4.1',$data1,'{$_SESSION['username']}')";

            $sql_insert_datarow_2 = "INSERT INTO physical_resource_management_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','7.4.2',$data2,'{$_SESSION['username']}')";
...
$sql = "SELECT  Marks FROM physical_resource_management_main WHERE Activity_Number ='7.4.1' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
...
$sql = "SELECT  Marks FROM physical_resource_management_main WHERE Activity_Number ='7.4.2' && School_ID ='{$_SESSION['school_id']}'  LIMIT 1 ";
$result = $con->query($sql);
...
<p>06 marks - Excellent </p>
  ...
  <tr class="active-row" >
    <td>7.4.1</td>
    <td>    Updating and maintaining main inventory and relevant sub - 
inventories properly (equipment are included in proper inventories and 
stock books are well balanced)</td>
   <td><input type="number"  autocomplete="off" name="marks1" value="<?php echo $marks1; ?>" required ></td>
    ...
    <tr class="active-row" >
    <td>7.4.2</td>
    <td>The zonal education office is made aware of board of surveys in 
correct time and action is taken to implement recommendations</td>
    ...
    <?php $_SESSION["physical.7_4"] = $total_marks; ?>
    <?php echo "Marks Total"."  ". $_SESSION["physical.7_4"]; ?>
  </tr>
...
<a  style="float:right; color: black"; href=" physical.7.5.php" >Go to next page</a>
<a  style="float:left; color: black;" href="physical.7.3.php" >Go to previous page</a>

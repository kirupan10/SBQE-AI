<?php session_start();
    ...
    if(isset($_POST['submit'])){
        ...
        if ($data1 != "" && $data2 != "" ){
            $sql_insert_datarow_1 = "INSERT INTO    co_curricular_activities_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','4.3.1',$data1,'{$_SESSION['username']}')";

            $sql_insert_datarow_2 = "INSERT INTO    co_curricular_activities_main (School_ID,Activity_Number,Marks,Session_Name) VALUE ('{$_SESSION['school_id']}','4.3.2',$data2,'{$_SESSION['username']}')";
...
Table – 1.2.2.2</P>
  ...
  <tr class="active-row" >
    <td>4.3.1</td>
    <td>Utilizing a transparent selection methodology to select students 
for each activity and providing opportunities for students to engage in 
different sports depending on their abilities and preferences.s</td>
    ...
    <tr class="active-row" >
    <td>4.3.2</td>
    <td>   Examining the physical fitness of students before allowing them 
to take part in sports competitions and enabling students to take part in 
competitions after being properly trained</td>
